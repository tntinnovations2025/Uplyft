...
<section>
    ...
    <form method="post" action="{{ route('profile.platform-logo.update') }}" enctype="multipart/form-data">
        @csrf

        {{-- Current Master Logo Status --}}
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:16px 20px;margin-bottom:18px">
                <div style="display:flex;align-items:center;gap:14px">
                    <div style="width:54px;height:54px;border-radius:12px;background:#ffffff;border:1.5px solid #cbd5e1;padding:3px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.06);overflow:hidden;flex-shrink:0">
                        <img src="{{ asset('storage/'.$currentPlatformLogo) }}" alt="UPLYFT Master Logo" style="width:100%;height:100%;object-fit:contain;border-radius:8px" />
                    </div>
                    <div>
        ...
        <div id="platform-logo-dropzone" 
            ...
            <div id="platform-dropzone-idle">
                ...
                <div style="font-size:14px;font-weight:800;color:#0f172a">{{ $currentPlatformLogo ? 'Click or Drop Image to Replace UPLYFT Master Logo' : 'Click or Drop Image to Set UPLYFT Master Logo' }}</div>
...
{{-- ── Interactive Circular Cropper Modal for Platform Logo (Apple Liquid Glass) ── --}}
