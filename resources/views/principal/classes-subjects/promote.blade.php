...
<div style="max-width:1100px;margin:0 auto;padding-bottom:60px">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:16px">
        <div>
            <div style="display:flex;align-items:center;gap:10px">
                <a href="{{ route('principal.classes-subjects.index') }}" class="btn btn-ghost btn-sm" style="color:var(--text-muted);border-color:rgba(255,255,255,0.1)">
        ...
        <div style="display:flex;gap:12px">
            <a href="{{ route('principal.classes-subjects.index') }}" class="btn btn-ghost">
                Cancel
            </a>
    ...
    <form id="promotionForm" method="POST" action="{{ route('principal.classes.promote') }}">
        @csrf
                ...
                <div class="form-group" style="margin-bottom:16px">
                    ...
                    <select id="promote_source_session_id" name="source_session_id" onchange="filterPromoteSourceClasses(this.value)" style="border-color:rgba(16,185,129,0.4);background:#0b0f19">
                        @foreach($allTerms as $t)
                            <option value="{{ $t->id }}" {{ $activeTerm && $activeTerm->id == $t->id ? 'selected' : '' }}>
                ...
                <div class="form-group" style="margin-bottom:0">
                    ...
                    <select id="promote_source_class_id" name="source_class_id" required onchange="previewFailedStudents()" style="border-color:rgba(16,185,129,0.4);background:#0b0f19">
                        ...
                        @foreach($allClasses as $c)
                            <option value="{{ $c->id }}" data-term="{{ $c->academic_term_id }}" {{ $selectedSourceClassId == $c->id ? 'selected' : '' }}>
                                🎓 {{ $c->custom_name }} ({{ $c->sections->count() }} Sections, {{ $c->students_count ?? 0 }} Students)
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" style="margin-bottom:16px">
                    ...
                    <select id="promote_target_class_id" name="target_class_id" style="border-color:rgba(56,189,248,0.4);background:#0b0f19">
                        ...
                        @foreach($allClasses as $ac)
                            <option value="{{ $ac->id }}" data-term="{{ $ac->academic_term_id }}">
                                {{ $ac->custom_name }} (Session: {{ $ac->academicTerm ? $ac->academicTerm->name : 'N/A' }})
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:20px;background:#121827;border:1px solid var(--border);border-radius:16px">
            <a href="{{ route('principal.classes-subjects.index') }}" class="btn btn-ghost" style="color:var(--text-muted)">
