...
@section('title', 'Exam Datesheets')
@section('page-header', 'Exam Datesheet')
    ...
    @if($examSchedules->isEmpty())
        <div class="liquid-glass-card p-12 text-center border border-dashed border-slate-300 shadow-2xs">
            ...
            <p class="text-xs text-slate-500 mt-1 max-w-md mx-auto font-medium">
                ...
                <strong class="text-slate-700">{{ $classSection?->instituteClass?->custom_name }} - {{ $classSection?->section_name }}</strong> by the administration yet.
            </p>
        </div>
    @else
        <div class="liquid-glass-card p-6 border border-slate-200/90 shadow-2xs">
            <div class="flex items-center justify-between mb-4 border-b border-slate-200 pb-3">
                ...
                <span class="badge badge-pink text-xs font-bold">
                    {{ $examSchedules->count() }} Exam Papers Scheduled
                </span>
            </div>
                        ...
                        @foreach($examSchedules as $exam)
                            ...
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="py-3.5 px-4">
                                    <div class="font-bold text-indigo-700 font-mono">⏱️ {{ $startTime }} - {{ $endTime }}</div>
                                    <div class="text-[11px] text-slate-500 font-medium mt-0.5">{{ $exam->duration_minutes ?: 60 }} Mins</div>
                                </td>
                                <td class="py-3.5 px-4">
                                    <div class="font-extrabold text-slate-900">📘 {{ $exam->subject->subject_name ?? 'Subject' }}</div>
                                    <div class="text-[11px] text-slate-500 font-medium">{{ $exam->subject->subject_code ?? '' }}</div>
                                </td>
                                <td class="py-3.5 px-4">
                                    <div class="font-bold text-slate-800">{{ $exam->title }}</div>
                                    @if($exam->instructions)
                                        <div class="text-[11px] text-slate-500 mt-0.5 max-w-xs truncate font-medium">{{ $exam->instructions }}</div>
                                    @endif
                                </td>
                                <td class="py-3.5 px-4">
                                    <span class="badge {{ $typeBadge }} text-[11px] font-extrabold uppercase">
