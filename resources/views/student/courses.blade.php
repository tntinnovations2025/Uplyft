...
@section('title', 'Course Catalog & Subjects')
@section('page-header', 'Course Catalog')
    ...
    <div class="liquid-glass-card p-6 bg-gradient-to-r from-pink-500/[0.06] via-purple-500/[0.04] to-indigo-500/[0.06] border border-slate-200/90 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-2xs glass-specular-top">
        ...
        <div class="flex items-center gap-2">
            <span class="badge badge-pink text-xs font-bold">
                <i class="fa-solid fa-layer-group"></i> {{ $subjects->count() }} Active {{ Str::plural('Subject', $subjects->count()) }}
            </span>
        </div>
    </div>
                ...
                <div class="liquid-glass-card p-6 flex flex-col gap-4 relative overflow-hidden group border border-slate-200/90 hover:border-pink-300 shadow-2xs hover:shadow-md transition-all duration-300 hover:-translate-y-1 bg-white">
                    <div class="flex items-start justify-between relative z-10">
                        <span class="badge badge-pink font-mono text-[10px] font-extrabold uppercase tracking-wider">
                            {{ $subject->subject_code ?? 'SUB-'.str_pad($subject->id, 3, '0', STR_PAD_LEFT) }}
                        </span>
                        @if($subject->credit_hours)
                            <span class="text-[11px] text-slate-500 font-bold flex items-center gap-1">
                                <i class="fa-solid fa-clock text-pink-600"></i>{{ $subject->credit_hours }} Cr. Hrs
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-4 text-xs text-slate-500 font-medium border-t border-slate-100 pt-3 mt-auto relative z-10">
                        <div class="flex items-center gap-1.5">
                            ...
                            <span><strong class="text-slate-800">{{ $subject->materials_count ?? $subject->materials->count() }}</strong> Books / Notes</span>
                        </div>
                    </div>

                    <div class="pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 relative z-10">
                        <a href="{{ route('lms.chatbot.index', ['subject_id' => $subject->id]) }}" class="py-2 px-3 rounded-xl bg-slate-100 hover:bg-pink-50 text-slate-700 hover:text-pink-700 text-xs font-bold text-center transition flex items-center justify-center gap-1.5 border border-slate-200/80">
                            <i class="fa-solid fa-robot text-pink-600"></i> AI Tutor
                        </a>
                        <a href="{{ route('lms.practice-test.index', ['subject_id' => $subject->id]) }}" class="py-2 px-3 rounded-xl btn-primary text-xs font-bold text-center transition flex items-center justify-center gap-1.5 shadow-2xs">
