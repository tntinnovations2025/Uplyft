{{-- ── GLOBAL ALERT NOTIFICATIONS COMPONENT ── --}}
...
{{-- ── 1. CENTER SCREEN WARNING MODAL ── --}}
<div id="centerWarningModal" class="warning-modal-backdrop" style="display: {{ $hasWarning ? 'flex' : 'none' }};">
    <div class="warning-modal-card">
        ...
        <div id="centerWarningMessage" style="font-size:13px;color:#475569;line-height:1.55;margin-bottom:20px;font-weight:600">
            @if(session('warning'))
                ...
                @endphp
                @if(count($parts) > 1)
                    @php $headerText = array_shift($parts); @endphp
                    <div style="font-size:13px;font-weight:800;color:#0f172a;margin-bottom:12px;text-align:left;background:#fef3c7;padding:9px 12px;border-radius:10px;border:1px solid #fde68a;display:flex;align-items:center;gap:6px">
                        <i class="fa-solid fa-bolt text-amber-600"></i> {{ $headerText }}
                    </div>
                ...
                @endif
            @elseif(!empty($errors) && method_exists($errors, 'any') && $errors->any())
        ...
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;width:100%;margin:0 auto">
            {{-- Green Blinking Border "Take me there" Action Button (Reduced size & centered in the middle) --}}
            <a href="{{ $warningUrl ?: route('principal.academic-terms.index') }}" id="centerWarningActionBtn" class="btn-take-me-there" style="{{ $warningUrl ? 'display:inline-flex;' : 'display:none;' }};margin:0 auto;align-items:center;gap:6px">
                <i class="fa-solid fa-location-arrow"></i> <span>Take me there</span> <i class="fa-solid fa-arrow-right"></i>
            </a>
            
            {{-- Dismiss Secondary Button (Compact & centered in the middle) --}}
...
{{-- ── 2. BOTTOM LINE SUCCESS TOAST BANNER (GREEN) ── --}}
@if(session('success'))
    <div id="bottomSuccessToast" class="toast-bottom-banner toast-success">
        <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
                ...
                <div style="font-size:13px;font-weight:700;margin-top:2px">{{ session('success') }}</div>
            </div>
        </div>
...
{{-- ── 3. BOTTOM LINE ERROR TOAST BANNER (RED) ── --}}
@if(session('error'))
    <div id="bottomErrorToast" class="toast-bottom-banner toast-error">
        <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
                ...
                <div style="font-size:13px;font-weight:700;margin-top:2px">{{ session('error') }}</div>
            </div>
        </div>
...
{{-- ── GLOBAL ALERT & TOAST JAVASCRIPT API ── --}}
<script>
