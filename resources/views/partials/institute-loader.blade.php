...
<div id="uplyft-global-loader" class="uplyft-loader-screen" aria-hidden="true" role="status" aria-label="Loading Portal Session">
    ...
    {{-- Pure Plain White Background with Soft Luminous Azure Aura --}}
    ...
    <div class="uplyft-loader-content">
        ...
        {{-- Middle: 3D Standing Coin Stage (Zooms In -> 360° Counter-Clockwise Spin -> Moves Backwards) --}}
        <div class="uplyft-coin-stage">
            
            {{-- Dual Celestial Orbit Energy Rings in Glossy Cyan & Gold --}}
            <div class="uplyft-orbit-ring uplyft-orbit-ring-primary"></div>
            <div class="uplyft-orbit-ring uplyft-orbit-ring-secondary"></div>

            {{-- Floor Surface Specular Shadow directly beneath the coin --}}
            <div class="uplyft-coin-shadow"></div>

            {{-- Multi-layered Volumetric Gold & Sky Blue Ion Aura --}}
            <div class="uplyft-logo-ion-aura"></div>
            <div class="uplyft-logo-pulse-halo"></div>

            {{-- 3D Standing Straight Upright Coin Spinning Frame with 24k Gold Bevel & Specular Sheen --}}
            <div class="uplyft-logo-frame uplyft-coin-spinning">
                {{-- Specular Reflection Gleam Sweep --}}
                <div class="uplyft-coin-specular-sweep"></div>

                @if(!empty($spinnerLogoUrl))
                    <img src="{{ $spinnerLogoUrl }}" 
                         alt="{{ $spinnerAltText }}" 
                ...
                {{-- Fallback Initial Badge if no custom logo is uploaded --}}
                <div id="uplyft-loader-fallback-badge" 
                     class="uplyft-logo-fallback" 
                     style="{{ (!empty($spinnerLogoUrl)) ? 'display:none;' : 'display:flex;' }}">
                    <span class="uplyft-fallback-letter">{{ $spinnerInitial }}</span>
                </div>
            </div>
        </div>

        {{-- Directly Under Logo: 'POWERED BY TNT INNOVATIONS' & Synchronized Laser Progress Bar --}}
        <div class="uplyft-loader-under-logo-group">
            
            {{-- Sleek Centered Premium Attribution Badge --}}
            ...
            {{-- Precision Synchronized Glossy Laser Progress Bar --}}
