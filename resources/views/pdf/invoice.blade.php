...
<div class="container">
            ...
            <td class="logo-container">
                ...
                @else
                    <div class="logo-placeholder">
                        {{ $institute->name ?? 'Uplyft Institution' }}
                    </div>
                @endif
            </td>
            <td class="meta-container">
                <h1>Official Fee Voucher</h1>
                <p class="invoice-number">INV-{{ str_pad($invoice->id ?? $student->id, 6, '0', STR_PAD_LEFT) }}</p>
                <p><strong>Fee Period / Month:</strong> {{ $feeMonth ?? ($invoice->fee_month ?? now()->format('F Y')) }}</p>
                <p>Issued Date: {{ $issuedAt ?? now()->format('Y-m-d') }}</p>
                <p>Due Date: {{ isset($invoice->due_date) ? $invoice->due_date->format('Y-m-d') : now()->addDays(7)->format('Y-m-d') }}</p>
            </td>
        </tr>
    </table>
    ...
    <table class="details-table">
        ...
        <tbody>
            <tr>
                ...
                <td class="details-val"><strong>{{ $student->full_name }}</strong></td>
                <td class="details-label">Roll Number:</td>
                <td class="details-val"><strong>{{ $student->roll_number }}</strong></td>
            </tr>
            <tr>
                <td class="details-label">Class &amp; Section:</td>
                <td class="details-val">
                    @if($student->classSection && $student->classSection->instituteClass)
                        {{ $student->classSection->instituteClass->name }} — Sec {{ $student->classSection->section_name ?? $student->classSection->name ?? '' }}
                    @else
                        {{ $student->enrolled_program ?? 'General' }}
                    @endif
                </td>
                <td class="details-label">Father / Guardian:</td>
                <td class="details-val">{{ $student->father_guardian_name ?? 'N/A' }}</td>
            </tr>
            <tr>
                <td class="details-label">Guardian Contact:</td>
                <td class="details-val">{{ $student->guardian_phone ?? $student->phone ?? 'N/A' }}</td>
    ...
    <table class="ledger-table">
        <thead>
            <tr>
                ...
                <th class="right">Total ({{ $currencySymbol ?? $currency ?? 'PKR' }})</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    ...
                    @if(!empty($student->scholarship_name) || !empty($feeBreakdown['scholarship_percentage']))
                        <br><span style="font-size:10px;color:#059669;font-weight:bold">
                            🎓 Policy: {{ $student->scholarship_name ?? 'Scholarship' }} ({{ number_format($feeBreakdown['scholarship_percentage'], 0) }}% Off)
                        </span>
                    @endif
                </td>
                <td class="right">{{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['base_fee'], 2) }}</td>
                <td class="right" style="color:#059669">
                    @if(!empty($feeBreakdown['scholarship_amount']) && $feeBreakdown['scholarship_amount'] > 0)
                        - {{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['scholarship_amount'], 2) }}
                    @else
                        —
                    @endif
                </td>
                <td class="right">—</td>
                <td class="right">{{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['subtotal_after_scholarship'], 2) }}</td>
            </tr>

            @if(!empty($feeBreakdown['admission_fee']) && $feeBreakdown['admission_fee'] > 0)
            <tr>
                <td>
                ...
                <td class="right">{{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['admission_fee'], 2) }}</td>
                <td class="right">—</td>
                <td class="right">—</td>
                <td class="right">{{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['admission_fee'], 2) }}</td>
            </tr>
            ...
            @if(!empty($feeBreakdown['security_fee']) && $feeBreakdown['security_fee'] > 0)
            <tr>
                <td>
            ...
            <tr>
                <td>
                    ...
                    <br><span style="font-size:9px;color:#64748b">Applied rate: {{ number_format($feeBreakdown['tax_percentage'], 0) }}% ({{ strtolower($student->guardian_tax_status ?? 'filer') === 'filer' ? 'Filer' : 'Non-Filer' }})</span>
                </td>
                <td class="right">—</td>
                <td class="right">—</td>
                <td class="right">{{ number_format($feeBreakdown['tax_percentage'], 0) }}%</td>
                <td class="right">{{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['tax_amount'], 2) }}</td>
            </tr>
            ...
            <tr class="total-row">
                ...
                <td class="right">{{ $currencySymbol ?? $currency ?? 'PKR' }} {{ number_format($feeBreakdown['grand_total'], 2) }}</td>
            </tr>
        </tbody>
    </table>

    @if((!empty($feeBreakdown['admission_fee']) && $feeBreakdown['admission_fee'] > 0) || (!empty($feeBreakdown['security_fee']) && $feeBreakdown['security_fee'] > 0))
    ...
    <!-- Page Footer -->
    <div class="footer">
        <p>This is an official system-generated electronic invoice issued by {{ $institute->name ?? 'UPLYFT Platform' }}.</p>
