...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
    ...
    <a href="{{ route('global-admin.institutes.index') }}" class="btn btn-secondary">← Back</a>
</div>

<div class="card" style="max-width:820px">
        {{-- ── Registration Type Selection (Single Institute vs Organization Network) ── --}}
        <div style="margin-bottom:24px;padding:20px;background:#f8fafc;border:1.5px solid #cbd5e1;border-radius:16px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                <label id="lbl-type-single" style="display:flex;align-items:flex-start;gap:12px;background:#ffffff;border:2px solid #4f46e5;border-radius:14px;padding:16px;cursor:pointer;transition:all 0.2s">
                    <input type="radio" name="registration_type" value="single" id="reg_type_single" {{ old('registration_type', 'single') === 'single' ? 'checked' : '' }} onchange="toggleRegTypeUI()" style="width:18px!important;height:18px!important;min-width:18px;margin-top:3px!important;margin-right:0!important;margin-bottom:0!important;margin-left:0!important;padding:0!important;accent-color:#4f46e5;flex-shrink:0;cursor:pointer;background:transparent;border:none;outline:none">
                    <div>
                ...
                <label id="lbl-type-org" style="display:flex;align-items:flex-start;gap:12px;background:#ffffff;border:2px solid #cbd5e1;border-radius:14px;padding:16px;cursor:pointer;transition:all 0.2s">
                    <input type="radio" name="registration_type" value="organization" id="reg_type_org" {{ old('registration_type') === 'organization' ? 'checked' : '' }} onchange="toggleRegTypeUI()" style="width:18px!important;height:18px!important;min-width:18px;margin-top:3px!important;margin-right:0!important;margin-bottom:0!important;margin-left:0!important;padding:0!important;accent-color:#4f46e5;flex-shrink:0;cursor:pointer;background:transparent;border:none;outline:none">
                    <div>
            ...
            {{-- Organization Sub-options Panel --}}
            <div id="org-details-panel" style="display:{{ old('registration_type') === 'organization' ? 'block' : 'none' }};margin-top:16px;padding:16px;background:#ffffff;border:1px solid #c7d2fe;border-radius:12px">
                ...
                <div style="display:flex;gap:16px;margin-bottom:14px">
                    <label style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:#0f172a;cursor:pointer">
                        <input type="radio" name="organization_mode" value="new" id="org_mode_new" {{ old('organization_mode', 'new') === 'new' ? 'checked' : '' }} onchange="toggleOrgModeUI()" style="width:16px!important;height:16px!important;min-width:16px;margin:0!important;padding:0!important;accent-color:#4f46e5;flex-shrink:0;cursor:pointer;background:transparent;border:none;outline:none">
                        Create New Organization Network
                    </label>
                    <label style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:#0f172a;cursor:pointer">
                        <input type="radio" name="organization_mode" value="existing" id="org_mode_existing" {{ old('organization_mode') === 'existing' ? 'checked' : '' }} onchange="toggleOrgModeUI()" style="width:16px!important;height:16px!important;min-width:16px;margin:0!important;padding:0!important;accent-color:#4f46e5;flex-shrink:0;cursor:pointer;background:transparent;border:none;outline:none">
                        Add to Existing Organization
                    </label>
                </div>

                {{-- New Organization Name & Campus Limit Quota --}}
                <div id="org-new-box" style="margin-bottom:0">
                    <div style="display:grid;grid-template-columns:2fr 1fr;gap:14px">
                        <div class="form-group" style="margin-bottom:0">
                            <label for="organization_name">Organization / Chain Name *</label>
                            <input id="organization_name" type="text" name="organization_name" value="{{ old('organization_name') }}" placeholder="e.g. Beaconhouse Educational Group, City Schools Network" />
                        </div>

                        <div class="form-group" style="margin-bottom:0">
                            <label for="max_campuses_new">Allowed Campuses Limit *</label>
                            <input id="max_campuses_new" type="number" name="max_campuses" min="1" max="50" value="{{ old('max_campuses', 3) }}" placeholder="e.g. 3" />
                        ...
                        <div class="form-group" style="margin-bottom:0">
                            ...
                            <select id="organization_id" name="organization_id" onchange="onExistingOrgSelected()">
                                ...
                                @foreach($organizations as $org)
                                    <option value="{{ $org->id }}" 
                                            data-owner-name="{{ $org->owner ? $org->owner->name : '' }}" 
                                            data-owner-email="{{ $org->owner ? $org->owner->email : '' }}"
                                            data-max-campuses="{{ $org->max_campuses ?? 3 }}"
                                            data-used-campuses="{{ $org->campus_count }}"
                                            data-can-add="{{ $org->canAddMoreCampuses() ? '1' : '0' }}"
                                            {{ old('organization_id') == $org->id ? 'selected' : '' }}>
                                        {{ $org->name }} ({{ $org->campus_usage_text }}) @if($org->owner)— Owner: {{ $org->owner->name }}@endif
                                    </option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
            ...
            <div class="form-group">
                ...
                @error('subscription_tier')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
            </div>
            ...
            <div class="form-group">
                ...
                @error('contact_email')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
            </div>
        ...
        {{-- ── Unified Institutional Brand Asset & Interactive Circular Cropper Studio ── --}}
        <div style="margin-top:16px;margin-bottom:24px;padding:22px;background:#ffffff;border:1.5px solid #e2e8f0;border-radius:16px;box-shadow:0 2px 8px rgba(15,23,42,0.04)">
            ...
            @error('logo')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
        </div>

        {{-- ── Interactive Circular Cropper Modal (Apple Liquid Glass) ── --}}
        <div id="circularCropModal" class="apple-liquid-glass-overlay" style="display:none;">
            <div class="apple-liquid-glass-card" style="max-width:540px;width:92%;padding:26px 30px;">
                ...
                {{-- Interactive Cropping Canvas Container --}}
        ...
        <div class="form-group" style="margin-top:8px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                ...
                <label style="
                    ...
                    {{ in_array($value, old('education_systems', [])) ? 'border-color:#e1306c;background:#fdf2f8' : '' }}
                ">
                    <input type="checkbox"
                    ...
                    <span style="font-size:13.5px;font-weight:700;color:#0f172a">{{ $label }}</span>
                </label>
                @endforeach
            </div>
            @error('education_systems')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:8px">{{ $message }}</p>@enderror
        </div>
        ...
        <div style="margin-top:28px;padding:24px;background:#fdf2f8;border:1.5px solid #fbcfe8;border-radius:14px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                <div class="form-group" style="grid-column:1/-1">
                    ...
                    @error('principal_name')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @error('principal_email')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @error('principal_password')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @error('principal_identifier')<p class="form-error" style="color:var(--danger);font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>
        ...
        <div style="display:flex;gap:12px;margin-top:24px">
            ...
            <a href="{{ route('global-admin.institutes.index') }}" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
</div>
@endsection
