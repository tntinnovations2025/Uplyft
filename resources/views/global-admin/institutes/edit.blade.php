...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
    ...
    <a href="{{ route('global-admin.institutes.show', $institute) }}" class="btn btn-secondary">← Back</a>
</div>
...
<div class="card" style="max-width:820px">
    <form method="POST" action="{{ route('global-admin.institutes.update', $institute) }}" enctype="multipart/form-data">
        ...
        <div style="margin-top:16px;margin-bottom:24px;padding:22px;background:#ffffff;border:1.5px solid #e2e8f0;border-radius:16px;box-shadow:0 2px 8px rgba(15,23,42,0.04)">
                ...
                <div id="current-logo-card" style="display:flex;align-items:center;justify-content:space-between;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:12px 16px;margin-bottom:14px">
                    <div style="display:flex;align-items:center;gap:14px">
                        <div style="width:48px;height:48px;border-radius:50%;background:#ffffff;border:2px solid #6366f1;padding:2px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 6px rgba(0,0,0,0.06);overflow:hidden;flex-shrink:0">
                            <img src="{{ asset('storage/'.$institute->logo_path) }}" alt="Current Logo" style="width:100%;height:100%;object-fit:contain;border-radius:50%" />
                        </div>
                        <div>
            ...
            <div id="brand-logo-dropzone" 
                ...
                <div id="dropzone-idle-state">
                    ...
                    <div style="font-size:13.5px;font-weight:800;color:#1e1b4b">{{ $institute->logo_path ? 'Click or Drop to Replace Institute Logo' : 'Click or Drop Image to Upload Full Logo' }}</div>
        ...
        {{-- ── Organization Network & Multi-Campus Architecture ── --}}
        <div style="margin-top:24px;margin-bottom:24px;padding:22px;background:#ffffff;border:1.5px solid #e2e8f0;border-radius:16px;box-shadow:0 2px 10px rgba(15,23,42,0.03)">
            ...
            <div class="form-group">
                ...
                <select id="organization_mode_edit" name="organization_assignment_type" onchange="toggleEditOrgUI()" style="width:100%">
                    <option value="none" {{ !$institute->organization_id ? 'selected' : '' }}>Single Standalone Institute (No Organization)</option>
                    <option value="existing" {{ $institute->organization_id ? 'selected' : '' }}>Attach to Existing Organization</option>
                    <option value="new">Convert &amp; Create New Organization Network</option>
                </select>
            </div>

            {{-- Existing Organization Box --}}
            <div id="edit-org-existing-box" style="display:{{ $institute->organization_id ? 'block' : 'none' }};margin-top:14px">
                ...
                <select id="edit_organization_id" name="organization_id" style="width:100%">
                    ...
                    @foreach($organizations as $org)
                        <option value="{{ $org->id }}" {{ $institute->organization_id == $org->id ? 'selected' : '' }}>
            ...
            {{-- New Organization Conversion Box --}}
            <div id="edit-org-new-box" style="display:none;margin-top:14px">
                <div style="display:grid;grid-template-columns:2fr 1fr;gap:14px">
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <input id="edit_organization_name" type="text" name="new_organization_name" value="{{ old('new_organization_name', $institute->name . ' Group') }}" placeholder="e.g. Crescent Educational Network" />
                    </div>
                    <div class="form-group" style="margin-bottom:0">
        ...
        <div class="form-group">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                ...
                <label style="
                    ...
                    background:#f8fafc;
                    border:1.5px solid {{ in_array($value, $currentSystems) ? '#6366f1' : '#cbd5e1' }};
                    border-radius:12px;padding:14px 16px;cursor:pointer;
                    background:{{ in_array($value, $currentSystems) ? '#eef2ff' : '#ffffff' }};
        ...
        <div style="display:flex;gap:12px;margin-top:28px">
            ...
            <a href="{{ route('global-admin.institutes.show', $institute) }}" class="btn btn-secondary"><i class="fa-solid fa-xmark"></i> Cancel</a>
        </div>
    </form>
</div>
@endsection
