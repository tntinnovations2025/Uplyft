...
@section('page-subtitle', 'Execute password reset or deny request for user credential recovery')
...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div class="card" style="margin-bottom: 24px;">
        <div class="card-header">
            ...
            <span class="badge {{ $notification->isPending() ? 'badge-amber' : 'badge-purple' }}">
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; font-size: 13px;">
            <div>
                ...
                <div style="color: #0f172a; font-weight: 800; font-size: 15px; margin-top: 4px;">{{ $notification->user->name }}</div>
            </div>
            <div>
            ...
            <div>
                ...
                <div style="margin-top: 4px;"><span class="badge badge-purple">{{ strtoupper($notification->user->role) }}</span></div>
            </div>
            <div>
                <div style="color: #64748b; font-size: 11px; text-transform: uppercase; font-weight: 700;">Institute</div>
                <div style="color: #0f172a; font-weight: 700; margin-top: 4px;">{{ $notification->institute->name ?? '—' }}</div>
            </div>
            <div>
                <div style="color: #64748b; font-size: 11px; text-transform: uppercase; font-weight: 700;">Request Timestamp</div>
                <div style="color: #0f172a; font-weight: 700; margin-top: 4px;">{{ $notification->created_at->format('M d, Y h:i A') }}</div>
            </div>
        </div>
    </div>
    ...
    @if($notification->isPending())
        ...
        <div class="card" style="margin-bottom: 24px;">
            <div class="card-header">
                <div>
                    ...
                    <div class="card-subtitle">Set temporary or new security password for {{ $notification->user->name }}</div>
                </div>
            </div>

            <form method="POST" action="{{ route('global-admin.password-resets.execute', $notification) }}">
                @csrf
                ...
                <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: 24px;">
                    <a href="{{ route('global-admin.password-resets.index') }}" class="btn btn-secondary">Cancel</a>
        ...
        <div class="card" style="border-color: #fecaca; background: #fff;">
            ...
            <form method="POST" action="{{ route('global-admin.password-resets.deny', $notification) }}">
                @csrf

                <div class="form-group">
                    <label for="notes">Denial Reason / Administrative Notes</label>
                    <textarea id="notes" name="notes" rows="3" placeholder="Enter reason for denying this request (optional)...">{{ old('notes') }}</textarea>
                </div>
