                ...
                <div class="p-6 text-gray-900">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="{{ $request->isPending() ? 'bg-yellow-50' : '' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            {{ $request->user->role === 'principal' ? 'bg-purple-100 text-purple-800' : ($request->user->role === 'teacher' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800') }}">
                                            {{ ucfirst($request->user->role) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $request->institute->name ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        @if($request->isPending())
                                            <a href="{{ route(request()->user()->isGlobalAdmin() ? 'global-admin.password-resets.show' : 'principal.password-resets.show', $request) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No password reset requests found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
