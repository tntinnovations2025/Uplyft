...
@section('title', 'Assessment & Exam Creator')
@section('breadcrumb', 'Assessments')
...
{{-- Filters --}}
<div class="card">
    <form method="GET" action="{{ route('lms.assessments.index') }}" style="display:flex;flex-wrap:wrap;gap:16px;align-items:flex-end">
        ...
        <div class="form-group" style="flex:1;min-width:180px;margin:0">
            ...
            <select name="academic_term_id">
                ...
                @foreach($terms as $t)
                    <option value="{{ $t->id }}" @selected(request('academic_term_id') == $t->id)>{{ $t->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group" style="flex:1;min-width:180px;margin:0">
            ...
            <select name="class_section_id">
                ...
                @foreach($sections as $sec)
                    <option value="{{ $sec->id }}" @selected(request('class_section_id') == $sec->id)>{{ $sec->instituteClass?->custom_name ?? 'Class' }} - {{ $sec->section_name }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="card" style="padding:20px">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:12px">
            <div style="display:flex;gap:14px;align-items:flex-start">
                <div class="badge badge-purple" style="margin-top:2px">{{ ucfirst($assessment->type) }}</div>
                <div>
                    <h3 style="font-family:'Space Grotesk',sans-serif;font-size:16px;font-weight:700">{{ $assessment->title }}</h3>
                    <p class="muted" style="margin-top:4px">
                        {{ $assessment->subject->subject_name ?? '—' }} ·
                        {{ $assessment->classSection->section_name ?? '—' }} ·
                        {{ $assessment->academicTerm->name ?? '—' }}
                        @if($assessment->creator)
                            <br>Created by {{ $assessment->creator->name }}
                        @endif
                    </p>
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:8px;white-space:nowrap">
                ...
                @endphp
                <span class="badge {{ $statusClass }}">{{ str_replace('_', ' ', $assessment->status) }}</span>
            </div>
        </div>

        <div style="display:flex;gap:24px;flex-wrap:wrap" class="muted">
            <span>🎯 {{ $assessment->total_marks }} marks</span>
            <span>🔢 {{ $assessment->questions_count ?? $assessment->questions->count() }} questions</span>
            <span>🤖 {{ $assessment->evaluation_mode === 'ai' ? 'AI Evaluation' : 'Manual Evaluation' }}</span>
            @if($assessment->start_time)
                <span>🗓️ {{ $assessment->start_time->format('M d, H:i') }} → {{ $assessment->end_time?->format('M d, H:i') }}</span>
            @endif
            @if($assessment->result_deadline)
                <span>⏳ Result deadline: {{ $assessment->result_deadline->format('M d, Y') }}</span>
            @endif
        </div>

        @if($assessment->questions->isNotEmpty())
            <details style="margin-top:14px">
                <summary style="cursor:pointer;font-size:13px;font-weight:600;color:var(--accent)">View {{ $assessment->questions->count() }} question(s)</summary>
                <div style="margin-top:10px;padding:14px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;font-size:13px" class="muted">
                    @foreach($assessment->questions as $q)
                        <div style="margin-bottom:8px">
                            <b>[{{ $q->marks }} pts] {{ ucfirst($q->question_type) }}:</b> {{ $q->statement }}
                        </div>
                    @endforeach
                </div>
            </details>
        ...
        @if($isTeacher || $isPrincipal)
            <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:16px;padding-top:16px;border-top:1px solid var(--border)">
                <a href="{{ route('lms.test-results.marksheet', $assessment->id) }}" class="btn btn-primary btn-sm">👁️ View Marks</a>

                @if($assessment->status === 'draft')
                    <form method="POST" action="{{ route('lms.assessments.publish', $assessment) }}">
                        @csrf
                        <button type="submit" class="btn btn-ghost btn-sm">🚀 Publish</button>
                    </form>
                    <button class="btn btn-ghost btn-sm" onclick="openQuestionsModal({{ $assessment->id }}, '{{ $assessment->title }}')">➕ Add Questions</button>
                @endif

                @if($assessment->isMandatory() && $isPrincipal)
                    <button class="btn btn-ghost btn-sm" onclick="openScheduleModal({{ $assessment->id }}, '{{ $assessment->title }}')">🗓️ Set Exam Schedule</button>
                @endif

                @if($assessment->status === 'published' || $assessment->status === 'in_progress')
                    <form method="POST" action="{{ route('lms.assessments.autoGradeMcqs', $assessment) }}">
                        @csrf
                ...
                @if($assessment->status === 'draft')
                    <form method="POST" action="{{ route('lms.assessments.destroy', $assessment) }}"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                ...
                <div class="form-group">
                    ...
                    <select name="class_section_id" required>
                        @foreach($sections as $sec)
                            <option value="{{ $sec->id }}">{{ $sec->instituteClass?->custom_name ?? 'Class' }} - {{ $sec->section_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="type" required>
                        @foreach($types as $type)
                            @if(in_array($type, ['midterm', 'final', 'exam']) && !auth()->user()->isAdministration())
                                @continue
                            @endif
...
<div class="modal-backdrop" id="questions-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,0.75);backdrop-filter:blur(8px);z-index:999;align-items:center;justify-content:center;padding:20px">
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;width:100%;max-width:760px;padding:24px;box-shadow:0 20px 40px rgba(0,0,0,0.5);max-height:90vh;overflow-y:auto">
        ...
        <form method="POST" action="{{ url('/lms/assessments') }}/QUESTION_ID/questions" id="questions-form">
            @csrf
...
<div class="modal-backdrop" id="schedule-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,0.75);backdrop-filter:blur(8px);z-index:999;align-items:center;justify-content:center;padding:20px">
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;width:100%;max-width:480px;padding:24px;box-shadow:0 20px 40px rgba(0,0,0,0.5)">
        ...
        <form method="POST" action="{{ url('/lms/assessments/') }}/SCHEDULE_ID/schedule" id="schedule-form">
            @csrf
            <div class="form-group">
...
<script>
    ...
    async function submitAiGenerator(e) {
        ...
        try {
            const res = await fetch('{{ route('lms.assessments.generateAi') }}', {
                method: 'POST',
                headers: {
    ...
    async function submitPracticeQuiz(e) {
        ...
        try {
            const res = await fetch('{{ route('lms.assessments.practiceQuiz') }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
