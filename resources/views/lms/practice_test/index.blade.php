...
@section('title', 'AI Practice Tests')
@section('breadcrumb', 'AI Practice Tests')
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
    ...
    <div class="lg:col-span-7">
        <div class="liquid-glass-card p-6 border border-slate-200/90 shadow-2xs relative">
            ...
            <form method="POST" action="{{ route('lms.practice-test.generate') }}" class="space-y-6">
                @csrf

                {{-- Class Selection (For Principal / Administration or Multi-Class Teachers ONLY) --}}
                @if(!auth()->user()->isStudent() && ($isAdministration || $classes->count() > 1))
                        ...
                        <div class="relative">
                            <select name="class_id" id="practice-class-selector" onchange="onPracticeClassChange(this.value)" class="styled-select pr-10">
                                ...
                                @foreach($classes as $cls)
                                    <option value="{{ $cls->id }}" {{ (string)$selectedClassId === (string)$cls->id ? 'selected' : '' }}>
                    ...
                    <div id="practice-no-class-prompt" style="{{ (!$selectedClassId) ? 'display:block;' : 'display:none;' }}" class="p-4 rounded-xl bg-indigo-50/50 border border-dashed border-indigo-300 text-center">
                        ...
                        <div class="p-3.5 rounded-xl bg-indigo-50/70 border border-indigo-200/70 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm font-bold text-indigo-700">{{ $classes->pluck('name')->implode(', ') }}</div>
                                </div>
                            </div>
                            <span class="badge badge-indigo text-xs font-bold">
                ...
                {{-- Subject Selection (Visible only when class is selected or student enrolled) --}}
                <div id="practice-step2-subject-section" style="{{ (!auth()->user()->isStudent() && ($isAdministration || $classes->count() > 1) && !$selectedClassId) || (auth()->user()->isStudent() && $subjects->isEmpty()) ? 'display:none;' : 'display:block;' }}" class="space-y-2">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
                        ...
                        <span>{{ (!auth()->user()->isStudent() && ($isAdministration || $classes->count() > 1)) ? 'Step 2: Choose Subject *' : 'Select Enrolled Subject *' }}</span>
                    </label>
                    <div class="relative">
                        ...
                        <div class="p-4 rounded-xl bg-slate-50/80 border border-slate-200/90 hover:border-indigo-300 transition-all flex items-center justify-between gap-4 group">
                            <div>
                                ...
                                <div class="text-xs text-slate-500 font-medium mt-1 flex items-center gap-2">
                                    <span class="text-indigo-700 font-bold">{{ $test->subject->subject_name ?? 'Subject' }}</span>
                                    <span>•</span>
                            ...
                            <div class="text-right shrink-0">
                                @if($test->status === 'evaluated')
                                    <span class="badge badge-emerald text-xs font-bold block mb-1">
                                        {{ $test->obtained_marks }} / {{ $test->total_marks }} pts
                                    </span>
                                    <a href="{{ route('lms.practice-test.result', $test->id) }}" class="inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-700">
                                        <span>Scorecard</span>
                                        <i class="fa-solid fa-arrow-up-right-from-square text-[10px]"></i>
                                    </a>
                                @else
                                    <a href="{{ route('lms.practice-test.take', $test->id) }}" class="btn-primary px-3 py-1.5 text-xs font-bold inline-flex items-center gap-1 shadow-2xs">
                                        <span>Continue</span>
...
<script>
    const isPracticeAdmin = {{ $isAdministration ? 'true' : 'false' }};
    let currentPracticeClassId = {{ $selectedClassId ? (int)$selectedClassId : 'null' }};
    const allPracticeSubjects = @json($subjectsJson ?? []);
    ...
    function applyPracticeClassFilter(classId) {
        ...
        if (!isStudent && !classId && (isPracticeAdmin || {{ $classes->count() > 1 ? 'true' : 'false' }})) {
