        ...
        <form method="POST" action="{{ route('lms.datesheet.store') }}" onsubmit="return validateDatesheetForm(event)" style="padding:20px">
            ...
            <div style="margin-bottom:20px">
                @foreach($classSections as $cs)
                    ...
                    @endphp
                    <div id="class-block-{{ $cs->id }}" class="class-schedule-block" style="display:none;padding:16px;background:#1e293b;border:1px solid #334155;border-radius:10px">
                        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid #334155;flex-wrap:wrap;gap:10px">
                            <div>
                                ...
                                <span style="font-size:12px;color:#94a3b8">{{ $csSubjects->count() }} Subjects Enrolled</span>
                            </div>
                            <div style="display:flex;align-items:center;gap:12px">
                                <div style="display:flex;align-items:center;gap:10px;background:#0f172a;padding:5px 10px;border-radius:6px;border:1px solid #334155">
                                    <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:11px;color:#cbd5e1;font-weight:600">
                                        ...
                                        <input type="checkbox" name="class_visibility[{{ $cs->id }}][is_published_teacher]" value="1" checked style="accent-color:#6366f1">
                                        <span>Teacher Portal</span>
                                    </label>
                                    <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:11px;color:#cbd5e1;font-weight:600">
                                        <input type="hidden" name="class_visibility[{{ $cs->id }}][is_published_student]" value="0">
                                        <input type="checkbox" name="class_visibility[{{ $cs->id }}][is_published_student]" value="1" checked style="accent-color:#6366f1">
                                        <span>Student Portal</span>
                                    </label>
                                </div>
                                <span style="font-size:12px;color:#94a3b8">Start Date:</span>
                                <input type="date" id="auto-date-{{ $cs->id }}" value="{{ date('Y-m-d') }}" style="padding:5px 8px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#f8fafc;font-size:12px">
                                <button type="button" class="btn-action btn-outline" onclick="autoFillDatesForClass({{ $cs->id }})" style="font-size:11px">
                        ...
                        <div id="paper-rows-container-{{ $cs->id }}" style="display:flex;flex-direction:column;gap:12px">
                            @if($csSubjects->isNotEmpty())
                                @foreach($csSubjects as $idx => $sub)
                                    <div class="paper-row-item-{{ $cs->id }}" style="background:#0f172a;border:1px solid #334155;border-radius:8px;padding:12px">
                                        <div style="display:grid;grid-template-columns:2.5fr 1.5fr 2fr 1fr 30px;gap:10px;align-items:center;margin-bottom:8px">
                                            <div>
                                                ...
                                                <select name="class_schedules[{{ $cs->id }}][{{ $idx }}][subject_id]" style="width:100%;padding:7px 10px;background:#1e293b;border:1px solid #334155;border-radius:6px;color:#ffffff;font-weight:600;font-size:12px">
                                            ...
                                            <div>
                                                ...
                                                <input type="date" name="class_schedules[{{ $cs->id }}][{{ $idx }}][exam_date]" class="cs-exam-date-{{ $cs->id }}" value="{{ date('Y-m-d', strtotime("+{$idx} days")) }}" style="width:100%;padding:7px 8px;background:#1e293b;border:1px solid #334155;border-radius:6px;color:#f8fafc;font-weight:600;font-size:12px">
                                            </div>
                                            <div>
                                                <label style="font-size:10px;font-weight:700;color:#94a3b8;display:block;margin-bottom:2px">TIME SLOT</label>
                                                <div style="display:flex;gap:4px">
                                                    <input type="time" name="class_schedules[{{ $cs->id }}][{{ $idx }}][start_time]" value="09:00" style="width:50%;padding:6px 4px;background:#1e293b;border:1px solid #334155;border-radius:6px;color:#f8fafc;font-size:11px">
                                                    <input type="time" name="class_schedules[{{ $cs->id }}][{{ $idx }}][end_time]" value="11:30" style="width:50%;padding:6px 4px;background:#1e293b;border:1px solid #334155;border-radius:6px;color:#f8fafc;font-size:11px">
                                                </div>
                                            </div>
                                            <div>
                                                <label style="font-size:10px;font-weight:700;color:#94a3b8;display:block;margin-bottom:2px">MARKS</label>
                                                <input type="number" name="class_schedules[{{ $cs->id }}][{{ $idx }}][total_marks]" value="100" min="1" max="1000" style="width:100%;padding:7px 8px;background:#1e293b;border:1px solid #334155;border-radius:6px;color:#f8fafc;font-weight:600;font-size:12px">
                                            </div>
                                            <div style="text-align:center;padding-top:14px">
                                                <button type="button" onclick="this.closest('.paper-row-item-{{ $cs->id }}').remove()" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:14px">✕</button>
                                            </div>
                                        </div>
                                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;align-items:center;padding:8px 10px;background:#1e293b;border-radius:6px">
                                            <div style="display:flex;align-items:center;gap:6px">
                                                ...
                                                <select name="class_schedules[{{ $cs->id }}][{{ $idx }}][room]" class="cs-room-select-{{ $cs->id }}" style="width:100%;padding:5px 8px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#ffffff;font-size:11px">
                                                    {!! $roomOptionsHtml !!}
                                                </select>
                                            </div>
                                            <div style="display:flex;align-items:center;gap:6px">
                                                <label style="font-size:10px;font-weight:700;color:#94a3b8">DEADLINE:</label>
                                                <input type="date" name="class_schedules[{{ $cs->id }}][{{ $idx }}][result_deadline]" class="cs-res-date-{{ $cs->id }}" value="{{ date('Y-m-d', strtotime("+".($idx+7)." days")) }}" style="width:100%;padding:5px 6px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#f8fafc;font-size:11px">
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div style="margin-top:12px">
                            <button type="button" class="btn-action btn-outline" onclick="addPaperRowForClass({{ $cs->id }})" style="font-size:11px">
...
<div class="card no-print" style="margin-bottom:20px;padding:14px;background:#0f172a;border:1px solid #1e293b;border-radius:10px">
    <form method="GET" action="{{ route('lms.datesheet.index') }}" style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;justify-content:space-between">
        ...
        @if(request()->has('class_section_id'))
            <a href="{{ route('lms.datesheet.index') }}" class="btn-action btn-outline" style="font-size:11px">Reset Filter</a>
        @endif
    </form>
</div>
...
<div id="classes-directory-container">
    @forelse($displayedSections as $cs)
        ...
        <div id="class-card-display-{{ $cs->id }}" class="class-datesheet-card">
            ...
            <div class="class-header-row" onclick="toggleClassBody({{ $cs->id }})">
                ...
                <div style="display:flex;align-items:center;gap:8px" onclick="event.stopPropagation()">
                    @if($exams->isNotEmpty())
                        ...
                        @endphp
                        <span class="badge-status {{ $classPubTeacher ? 'badge-complete' : 'badge-pending' }}" style="font-size:11px; {{ $classPubTeacher ? 'background:rgba(16,185,129,0.1);color:#10b981;border:1px solid rgba(16,185,129,0.3)' : 'background:rgba(239,68,68,0.1);color:#ef4444;border:1px solid rgba(239,68,68,0.3)' }}">
                            Teacher Portal: {{ $classPubTeacher ? 'Visible' : 'Hidden' }}
                        </span>
                        <span class="badge-status {{ $classPubStudent ? 'badge-complete' : 'badge-pending' }}" style="font-size:11px; {{ $classPubStudent ? 'background:rgba(16,185,129,0.1);color:#10b981;border:1px solid rgba(16,185,129,0.3)' : 'background:rgba(239,68,68,0.1);color:#ef4444;border:1px solid rgba(239,68,68,0.3)' }}">
                            Student Portal: {{ $classPubStudent ? 'Visible' : 'Hidden' }}
                        </span>
                    @endif
                    ...
                    @endif
                    <div id="accordion-arrow-{{ $cs->id }}" style="color:#94a3b8;margin-left:6px;transition:transform 0.2s ease">
            ...
            <div id="class-card-body-{{ $cs->id }}" class="class-card-body" style="display:none;border-top:1px solid #1e293b;padding:16px 20px">
                
                {{-- Unschedueled Subjects Alert Callout --}}
                @if($unscheduledSubjects->isNotEmpty())
                    <div style="margin-bottom:16px;padding:12px 16px;background:rgba(245, 158, 11, 0.08);border:1px solid rgba(245, 158, 11, 0.2);border-radius:8px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
                        ...
                        @if(auth()->user()->isAdministration())
                            <button type="button" class="btn-action btn-outline" onclick="quickCreateSubjectExam({{ $cs->id }})" style="font-size:11px;border-color:rgba(245, 158, 11, 0.4);color:#f59e0b">
                    ...
                    <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px" class="no-print">
                        ...
                        <div style="display:flex;align-items:center;gap:10px">
                            <form method="POST" action="{{ route('lms.datesheet.toggleVisibility', $cs->id) }}" style="margin:0">
                                ...
                                <button type="submit" class="btn-action {{ $classPubTeacher ? 'btn-danger-custom' : 'btn-primary-custom' }}" style="font-size:11px;padding:6px 14px">
                            ...
                            <form method="POST" action="{{ route('lms.datesheet.toggleVisibility', $cs->id) }}" style="margin:0">
                                ...
                                <button type="submit" class="btn-action {{ $classPubStudent ? 'btn-danger-custom' : 'btn-primary-custom' }}" style="font-size:11px;padding:6px 14px">
                    ...
                    <table class="datesheet-table">
                        ...
                        <tbody>
                            @foreach($exams as $idx => $exam)
                                ...
                                <tr>
                                    <td>
                                        <div style="font-weight:700;color:#f8fafc">{{ $exam->subject->subject_name ?? 'Subject' }}</div>
                                        <div style="font-size:11px;color:#64748b">{{ $exam->subject->subject_code ?? '' }}</div>
                                    </td>
                                    <td>
                                        <div style="font-weight:600;color:#f8fafc">{{ $startDate }}</div>
                                        <div style="font-size:11px;color:#818cf8">{{ $startTime }} - {{ $endTime }}</div>
                                        <div style="font-size:10px;color:#64748b">{{ $sessionLabel }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div style="font-weight:600;color:#f8fafc">{{ $exam->title }}</div>
                                        <div style="font-size:11px;color:#64748b">{{ $exam->total_marks }} Marks</div>
                                    </td>
                                    <td>
                                        <div style="font-size:11px;color:#f59e0b">{{ $resultDeadline }}</div>
                                    </td>
                                    @if(auth()->user()->isAdministration())
                                        <td class="no-print" style="text-align:right">
                                            <div style="display:flex;align-items:center;justify-content:flex-end;gap:6px">
                                                <button type="button" class="btn-action btn-outline" onclick="openEditModal({{ $exam->id }}, '{{ addslashes($exam->title) }}', '{{ $exam->type }}', {{ $exam->academic_term_id ?? 'null' }}, '{{ $rawExamDate }}', '{{ $rawStartTime }}', '{{ $rawEndTime }}', {{ $exam->total_marks }}, '{{ $rawDeadline }}', '{{ addslashes($exam->instructions ?? '') }}', '{{ addslashes($roomName) }}', {{ $pubTeacher ? 1 : 0 }}, {{ $pubStudent ? 1 : 0 }})" style="padding:3px 8px;font-size:11px">
                                                    Edit
                                                </button>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(200px, 1fr));gap:8px;padding:12px;background:#1e293b;border:1px solid #334155;border-radius:8px">
                        @foreach($classSections as $cs)
                            ...
                            <label style="display:flex;align-items:center;gap:8px;font-size:12px;color:#f8fafc;cursor:pointer;padding:8px;border-radius:6px;background:#0f172a;border:1px solid #334155">
                                <input type="checkbox" id="class-chk-{{ $cs->id }}" value="{{ $cs->id }}" data-class-name="{{ $cName }} - {{ $cs->section_name }}" class="class-builder-checkbox" style="accent-color:#6366f1">
                                <div>
                                    <div style="font-weight:600">{{ $cName }} - {{ $cs->section_name }}</div>
                                </div>
                            </label>
                        @endforeach
                    </div>
