...
@section('breadcrumb', 'Grade Reports')
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px">
    ...
    @if(!$isStudent)
        <a href="{{ route('lms.assessments.paperMarksheet.form') }}" class="btn btn-primary">
...
<div id="report-container" style="display:none">
    ...
    {{-- Student Report Card (for individual students) --}}
...
<script>
    ...
    async function loadStudentReport() {
        ...
        try {
            const res = await fetch(`/lms/grades/student-report?student_id={{ auth()->id() }}&subject_id=${subjectId}&academic_term_id=${termId}&class_section_id=${sectionId}`, {
    ...
    async function loadClassReport() {
        ...
        try {
            const res = await fetch(`/lms/grades/class-report?subject_id=${subjectId}&academic_term_id=${termId}&class_section_id=${sectionId}&student_ids[]={{ auth()->id() }}`, {
                headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken }
            });
            const data = await res.json();
