...
@section('title', $class->name . ' — Subject Materials')
@section('page-header', $class->name . ' — Subjects & RAG Materials')
...
<div>
    ...
    <div class="subj-breadcrumb">
        <a href="{{ route('lms.subjects.list') }}">📚 All Classes</a>
        <span class="sep">›</span>
        <span style="color: #1B1A17; font-weight: 700;">{{ $class->name }}</span>
    </div>

    <!-- Class Hero -->
    <div class="class-hero">
        <div style="display: flex; align-items: center; gap: 16px;">
            <div class="class-hero-icon">{{ strtoupper(substr($class->name, 0, 1)) }}</div>
            <div>
                <h1 style="font-family: 'Outfit', sans-serif; font-size: 21px; font-weight: 800; color: #1B1A17; margin: 0 0 4px;">
                    {{ $class->name }} — Subject Library
                </h1>
                <p style="font-size: 12.5px; color: #68665D; margin: 0; font-weight: 500;">
                    {{ $subjects->count() }} {{ Str::plural('subject', $subjects->count()) }} registered 
                    @if($class->sections->count() > 0)
                        • {{ $class->sections->count() }} {{ Str::plural('section', $class->sections->count()) }}
                        ({{ $class->sections->pluck('section_name')->join(', ') }})
                    @endif
                </p>
            </div>
        </div>
        <a href="{{ route('lms.subjects.list') }}" style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:10px;background:#F2EFEB;border:1px solid #E1DFD7;color:#68665D;font-size:12px;font-weight:700;text-decoration:none;transition:all 0.2s">
            ← Back to Classes
        </a>
    </div>
    ...
    @if($subjects->isEmpty())
        <div style="text-align: center; padding: 64px 24px; background: #F9F8F5; border: 1px dashed #E1DFD7; border-radius: 16px;">
            <div style="display: inline-flex; width: 64px; height: 64px; border-radius: 14px; align-items: center; justify-content: center; font-size: 28px; background: #F2EFEB; border: 1px solid #E1DFD7; margin-bottom: 14px; color: #8A877E;">
                <x-icon name="book-bookmark" class="w-7 h-7" />
            </div>
                ...
                <div class="subject-card">
                    <div class="subject-card-top">
                        <div class="subject-card-meta">
                            <span class="subject-card-code">{{ $subject->subject_code ?? 'SUB-'.$subject->id }}</span>
                            <span class="subject-card-files" style="color: {{ $fileCount > 0 ? '#2E6E42' : '#8A877E' }}; font-weight: 700;">
                                <x-icon name="file-lines" class="w-3.5 h-3.5" /> {{ $fileCount }} {{ Str::plural('File', $fileCount) }}
                            </span>
                        </div>
                        <div class="subject-card-name">{{ $subject->subject_name ?? 'Subject #'.$subject->id }}</div>
                        <div class="subject-card-class">
                            {{ $class->name }}
                            @if($subject->credit_hours)
                                • {{ $subject->credit_hours }} Credit {{ Str::plural('Hour', $subject->credit_hours) }}
                            @endif
                        </div>
                    </div>
                    <div class="subject-card-stats">
                        <div class="subject-stat">
                            <div class="subject-stat-num" style="color: #D48A2E;">{{ $textbooks }}</div>
                            <div class="subject-stat-label">Textbooks</div>
                        </div>
                        <div class="subject-stat">
                            <div class="subject-stat-num" style="color: #8A5A10;">{{ $notes }}</div>
                            <div class="subject-stat-label">Notes</div>
                        </div>
                        <div class="subject-stat">
                            <div class="subject-stat-num" style="color: #68665D;">{{ max($others, 0) }}</div>
                            <div class="subject-stat-label">Reference</div>
                        </div>
                    </div>
                    <div class="subject-card-actions">
                        <a href="{{ route('lms.materials.index', $subject->id) }}" class="subject-btn subject-btn-primary">
                            <x-icon name="cloud-arrow-up" class="w-4 h-4" /> Upload / Manage Books
                        </a>
                    </div>
                </div>
            @endforeach
