...
@if($href && !$disabled)
    <a href="{{ $href }}" {{ $attributes->merge(['class' => "{$baseClasses} {$sizeClasses} {$variantClasses}"]) }}>
        ...
        @elseif($icon && $iconPosition === 'left')
            <i class="{{ $icon }} transition-transform duration-200 group-hover:scale-110"></i>
        @endif

        <span class="relative z-10 font-medium">{{ $slot }}</span>

        @if(!$loading && $icon && $iconPosition === 'right')
            <i class="{{ $icon }} transition-transform duration-200 group-hover:translate-x-0.5"></i>
        @endif
    </a>
@else
    <button type="{{ $type }}" @if($disabled || $loading) disabled @endif {{ $attributes->merge(['class' => "{$baseClasses} {$sizeClasses} {$variantClasses}"]) }}>
        {{-- Specular Reflection Line --}}
        <div class="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-white/40 to-transparent pointer-events-none opacity-60 group-hover:opacity-100 transition-opacity"></div>
