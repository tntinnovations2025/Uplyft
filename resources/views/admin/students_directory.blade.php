...
<div class="space-y-6">
    ...
    <div class="liquid-glass-card p-6 flex flex-col md:flex-row items-center justify-between gap-4 border border-slate-200/90 shadow-2xs glass-specular-top">
        <div>
            ...
            <p class="text-xs text-slate-500 font-medium">Total Enrolled: <span class="badge badge-pink text-xs font-bold">{{ $students->count() }}</span></p>
        </div>
        <a href="{{ route('admissions.index') }}" class="btn-primary py-2.5 px-5 text-sm font-extrabold shadow-md inline-flex items-center gap-2">
                ...
                <tbody class="text-sm text-slate-700 divide-y divide-slate-100 bg-white">
                    @forelse($students as $student)
                        <tr class="hover:bg-slate-50 transition">
                            <td class="py-3.5 px-6 font-bold font-mono text-pink-700">{{ $student->roll_number }}</td>
                            <td class="py-3.5 px-6 font-extrabold text-slate-900">{{ $student->first_name }} {{ $student->last_name }}</td>
                            <td class="py-3.5 px-6 font-semibold text-slate-600">{{ $student->enrolled_program ?? 'N/A' }}</td>
                            <td class="py-3.5 px-6 font-medium text-slate-500">{{ $student->created_at->format('M d, Y') }}</td>
                        </tr>
                    @empty
                        <tr>
