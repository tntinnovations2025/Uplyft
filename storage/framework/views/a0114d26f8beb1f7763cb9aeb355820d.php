...
<?php $__env->startSection('title', 'Practice Test Results & AI Scorecard'); ?>
<?php $__env->startSection('breadcrumb', 'Test Scorecard'); ?>
...
<div class="mb-8 p-6 md:p-8 rounded-2xl liquid-glass-card bg-gradient-to-r from-pink-500/[0.06] via-purple-500/[0.04] to-indigo-500/[0.06] border border-slate-200/90 shadow-2xs relative overflow-hidden glass-specular-top">
    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            ...
            <h1 class="font-extrabold text-2xl md:text-3xl text-slate-900 tracking-tight font-display"><?php echo e($test->title); ?></h1>
        ...
        <div class="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-2xs shrink-0">
            <div class="text-right">
                ...
                <div class="font-extrabold text-3xl text-pink-700 font-mono font-display">
                    <?php echo e($test->obtained_marks); ?> <span class="text-sm font-normal text-slate-400">/ <?php echo e($test->total_marks); ?></span>
                </div>
            </div>
            <div class="px-4 py-2 rounded-xl text-lg font-black tracking-wide shadow-2xs badge <?php echo e($gradeBadgeClass); ?>">
...
<div class="liquid-glass-card p-6 border border-slate-200/90 shadow-2xs space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
        ...
        <a href="<?php echo e(route('lms.practice-test.index')); ?>" class="btn-primary px-4 py-2 text-xs font-bold shadow-sm inline-flex items-center gap-2 self-start sm:self-auto">
    ...
    <div class="space-y-4">
        <?php $__currentLoopData = ($test->evaluation_results ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $res): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="p-5 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4 hover:border-pink-300 transition-all">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3">
                        <span class="w-7 h-7 rounded-lg bg-slate-100 border border-slate-300 text-slate-800 flex items-center justify-center font-bold text-xs shrink-0 mt-0.5 font-display">
                            <?php echo e($res['question_index']); ?>

                        </span>
                        <div>
                            <h4 class="font-bold text-sm text-slate-900 leading-relaxed">
                                <?php echo e($res['question_text']); ?>
                            ...
                            <span class="inline-block mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-500 px-2 py-0.5 rounded bg-slate-100 border border-slate-200">
                                Type: <?php echo e(strtoupper($res['question_type'] ?? 'short')); ?>
                    ...
                    <div class="shrink-0">
                        <?php if(($res['obtained_marks'] ?? 0) == ($res['max_marks'] ?? 0)): ?>
                            <span class="badge badge-emerald text-xs font-bold">
                                +<?php echo e($res['obtained_marks']); ?> / <?php echo e($res['max_marks']); ?> pts
                            </span>
                        <?php elseif(($res['obtained_marks'] ?? 0) > 0): ?>
                            <span class="badge badge-amber text-xs font-bold">
                                +<?php echo e($res['obtained_marks']); ?> / <?php echo e($res['max_marks']); ?> pts
                            </span>
                        <?php else: ?>
                            <span class="badge badge-rose text-xs font-bold">
                                0 / <?php echo e($res['max_marks']); ?> pts
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    <div class="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                        ...
                        <div class="text-slate-800 font-medium whitespace-pre-wrap leading-relaxed">
                            <?php echo e($res['student_answer'] ?: '(No answer provided)'); ?>
                ...
                <div class="p-3.5 rounded-xl bg-purple-50/60 border-l-4 border-purple-500 text-xs text-purple-950 leading-relaxed flex items-start gap-2.5">
                    ...
                    <div>
                        ...
                        <span class="ml-1 text-slate-700 font-medium"><?php echo e($res['feedback']); ?></span>
                    </div>
                </div>
            </div>
...
<?php echo $__env->make('lms.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\UPLYFT\uplifyt\resources\views\lms\practice_test\result.blade.php ENDPATH**/ ?>