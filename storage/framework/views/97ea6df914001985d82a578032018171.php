...
<?php $__env->startSection('page-title', 'Authorize & Reset User Password'); ?>
<?php $__env->startSection('page-subtitle', 'Execute password reset or deny request for student/teacher account'); ?>
...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div class="card" style="margin-bottom: 24px; padding: 24px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;border-bottom:1px solid var(--border);padding-bottom:12px">
            ...
            <span class="badge" style="background:<?php echo e($notification->isPending() ? 'rgba(245,158,11,0.15)' : 'rgba(0,206,209,0.15)'); ?>;border:1px solid <?php echo e($notification->isPending() ? 'rgba(245,158,11,0.3)' : 'rgba(0,206,209,0.3)'); ?>;color:<?php echo e($notification->isPending() ? '#f59e0b' : '#00ced1'); ?>;padding:4px 10px;border-radius:6px;font-size:11px;font-weight:700">
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; font-size: 13px;">
            <div>
                ...
                <div style="color: #fff; font-weight: 700; font-size: 15px; margin-top: 4px;"><?php echo e($notification->user->name); ?></div>
            </div>
            <div>
                <div style="color: var(--text-muted); font-size: 11px; text-transform: uppercase; font-weight: 700;">Email / Account Identifier</div>
                <div style="color: #38bdf8; font-weight: 700; font-size: 14px; margin-top: 4px;"><?php echo e($notification->user->identifier ?? $notification->user->email); ?></div>
            </div>
            <div>
                ...
                <div style="margin-top: 4px;">
                    <span class="badge" style="background:rgba(56,189,248,0.15);border:1px solid rgba(56,189,248,0.3);color:#38bdf8;padding:4px 8px;border-radius:6px;font-size:11px;font-weight:700">
                        <?php echo e(strtoupper($notification->user->role)); ?>
            ...
            <div>
                ...
                <div style="color: #fff; margin-top: 4px;"><?php echo e($notification->created_at->format('M d, Y h:i A')); ?></div>
            </div>
        </div>
    </div>
    ...
    <?php if($notification->isPending()): ?>
        ...
        <div class="card" style="margin-bottom: 24px; padding: 24px;">
            <div style="margin-bottom:18px;border-bottom:1px solid var(--border);padding-bottom:12px">
                ...
                <span style="font-size:12px;color:var(--text-muted)">Set temporary or new security password for <?php echo e($notification->user->name); ?></span>
            </div>

            <form method="POST" action="<?php echo e(route($routePrefix . 'password-resets.execute', $notification)); ?>">
                <?php echo csrf_field(); ?>
...
$message = $__bag->first($__errorArgs[0]); ?>
                        <span style="color: #ef4444; font-size: 12px; display: block; margin-top: 4px;"><?php echo e($message); ?></span>
                    <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                ...
                <div style="display: flex; justify-content: flex-end; gap: 12px;">
                    <a href="<?php echo e(route($routePrefix . 'password-resets.index')); ?>" class="btn-secondary" style="padding:8px 16px;border-radius:8px;text-decoration:none;color:#94a3b8;font-size:13px">Cancel</a>
        ...
        <div class="card" style="padding: 24px; border-color: rgba(239, 68, 68, 0.3);">
            ...
            <form method="POST" action="<?php echo e(route($routePrefix . 'password-resets.deny', $notification)); ?>">
                <?php echo csrf_field(); ?>
                ...
                <div class="form-group" style="margin-bottom:16px">
                    ...
                    <textarea id="notes" name="notes" rows="3" placeholder="Enter reason for denying this request (optional)..." style="width:100%;padding:10px 14px;background:rgba(15,23,42,0.8);border:1px solid var(--border);border-radius:8px;color:#fff;font-size:13px"><?php echo e(old('notes')); ?></textarea>
                </div>
...
<?php echo $__env->make(auth()->check() && !auth()->user()->isPrincipal() && !auth()->user()->isGlobalAdmin() ? 'layouts.app' : 'principal.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\UPLYFT\uplifyt\resources\views\principal\password-resets\show.blade.php ENDPATH**/ ?>