<?php $__env->startSection('title', 'Faculty & Staff Directory'); ?>
<?php $__env->startSection('breadcrumb', 'Faculty & Staff Roster'); ?>
...
<div class="roster-header">
    ...
    <div style="display:flex;gap:10px;align-items:center">
        <?php if($canEditStaff): ?>
            <a href="<?php echo e(route('principal.staff.authorities')); ?>" class="btn btn-ghost" style="border-color:#cbd5e1;color:#0f172a">
            ...
            <a href="<?php echo e(route('principal.staff.create')); ?>" class="btn btn-primary">
...
<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:20px;flex-wrap:wrap">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        <button type="button" class="cat-tab active" onclick="filterStaffCategory('all', this)">
            <span>🌐</span> All Staff
            <span class="cat-pill" id="badge-all"><?php echo e($counts['all']); ?></span>
        </button>
        <button type="button" class="cat-tab" onclick="filterStaffCategory('teacher', this)">
            <span>👨‍🏫</span> Teachers &amp; Faculty
            <span class="cat-pill" id="badge-teacher"><?php echo e($counts['teacher']); ?></span>
        </button>
        <button type="button" class="cat-tab" onclick="filterStaffCategory('administration', this)">
            <span>💼</span> Administration
            <span class="cat-pill" id="badge-admin"><?php echo e($counts['administration']); ?></span>
        </button>
        <button type="button" class="cat-tab" onclick="filterStaffCategory('accountant', this)">
            <span>💰</span> Accountants
            <span class="cat-pill" id="badge-accountant"><?php echo e($counts['accountant']); ?></span>
        </button>
        <button type="button" class="cat-tab" onclick="filterStaffCategory('coordinator', this)">
            <span>📋</span> Coordinators
            <span class="cat-pill" id="badge-coordinator"><?php echo e($counts['coordinator']); ?></span>
        </button>

        <?php $__currentLoopData = $customCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cSlug => $cTitle): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button type="button" class="cat-tab" onclick="filterStaffCategory('custom-<?php echo e($cSlug); ?>', this)">
                <span>⭐</span> <?php echo e($cTitle); ?>

                <span class="cat-pill" id="badge-custom-<?php echo e($cSlug); ?>"><?php echo e($counts['custom-' . $cSlug] ?? 0); ?></span>
            </button>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

        <button type="button" class="cat-tab" id="tab-other" onclick="filterStaffCategory('other', this)" style="<?php echo e(($counts['other'] > 0) ? '' : 'display:none'); ?>">
            <span>🧩</span> Other Staff
            <span class="cat-pill" id="badge-other"><?php echo e($counts['other']); ?></span>
        </button>
    </div>
...
<div class="roster-card" style="overflow-x:hidden">
    <div style="width:100%">
        ...
        <tbody id="staff-table-body">
            ...
            ?>
            <tr class="staff-row" data-staff-category="<?php echo e($catKey); ?>" data-search-text="<?php echo e(strtolower($staff->name . ' ' . $staff->email . ' ' . ($staff->identifier ?? '') . ' ' . $roleTitle)); ?>">
                <td>
                    <div style="display:flex;align-items:center;gap:14px">
                        <div class="staff-avatar-circle" style="background:<?php echo e($avatarGradient); ?>;box-shadow:0 2px 8px rgba(0,0,0,0.1)">
                            <?php echo e(strtoupper(substr($staff->name, 0, 1))); ?>

                        </div>
                        <div>
                            <div style="font-weight:700;color:#0f172a;font-size:14.5px"><?php echo e($staff->name); ?></div>
                            <div style="font-size:12px;color:#64748b;margin-top:1px">✉️ <?php echo e($staff->email); ?></div>
                            <?php if($staff->identifier): ?>
                ...
                <td>
                    <span style="font-size:11px;font-weight:800;padding:5px 12px;border-radius:20px;letter-spacing:0.4px;display:inline-flex;align-items:center;gap:6px;<?php echo e($badgeStyle); ?>">
                        <span style="width:6px;height:6px;border-radius:50%;background:currentColor"></span>
                        <?php echo e(strtoupper($roleTitle)); ?>
                ...
                <td style="text-align:center">
                    <?php if($staff->is_primary_principal || ($staff->role === 'principal' && $staff->created_by === null)): ?>
                    ...
                    <?php elseif($staff->role === 'principal'): ?>
                    ...
                    <?php else: ?>
                        <span class="rights-badge" data-delegate-badge="<?php echo e($staff->id); ?>" style="font-size:11.5px;padding:6px 14px;border-radius:20px;font-weight:800;background:<?php echo e($staff->is_delegated_admin ? '#eef2ff' : '#f8fafc'); ?>;color:<?php echo e($staff->is_delegated_admin ? '#4338ca' : '#475569'); ?>;border:1px solid <?php echo e($staff->is_delegated_admin ? '#c7d2fe' : '#cbd5e1'); ?>">
                            <?php echo e($staff->is_delegated_admin ? '👑 Master Delegated Admin' : '🛡️ Standard Role Rights'); ?>
                ...
                <td style="text-align:right">
                    <?php if($staff->is_primary_principal && !auth()->user()->isGlobalAdmin()): ?>
                    ...
                    <?php elseif($staff->role !== 'global_admin'): ?>
                        <?php
                        ...
                        <div style="display:flex;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap">
                            <?php if($canEditStaff): ?>
                                ...
                                <button type="button" class="btn" onclick='openEditRoleModal(<?php echo e(json_encode($editRoleData, JSON_HEX_APOS | JSON_HEX_QUOT)); ?>)' title="Change Designation, Role & Contract Type" style="padding:6px 11px;font-size:11.5px;font-weight:800;border-radius:10px;background:#eff6ff;color:#2563eb;border:1.5px solid #bfdbfe;cursor:pointer">
                                    ✏️ Edit Role
                                </button>
                            <?php endif; ?>

                            
                            <button type="button" class="btn" onclick='openStaffDetailModal(<?php echo e(json_encode($staffDetailData, JSON_HEX_APOS | JSON_HEX_QUOT)); ?>)' title="View Employee Details & Uploaded Documents" style="padding:6px 11px;font-size:11.5px;font-weight:800;border-radius:10px;background:#f0fdf4;color:#16a34a;border:1.5px solid #bbf7d0;cursor:pointer">
                                👁️ Details
                            </button>

                            <?php if($canEditStaff): ?>
                                
                                <button type="button" class="btn btn-rights" data-staff-id="<?php echo e($staff->id); ?>" data-staff-perms='<?php echo e(json_encode($perms, JSON_HEX_APOS | JSON_HEX_QUOT)); ?>' data-staff-delegate="<?php echo e($staff->is_delegated_admin ? 1 : 0); ?>" onclick="openPermModal('<?php echo e($staff->id); ?>', '<?php echo e(addslashes($staff->name)); ?>', this)" style="padding:6px 12px;font-size:11.5px;font-weight:800;border-radius:10px;background:#eef2ff;color:#4f46e5;border:1.5px solid #c7d2fe;cursor:pointer">
                                    ⚙️ Rights
                                </button>

                                
                                <button type="button" class="btn" onclick="openDirectResetModal(<?php echo e($staff->id); ?>, '<?php echo e(addslashes($staff->name)); ?>', '<?php echo e(addslashes($staff->email)); ?>')" title="Direct Password Reset" style="padding:6px 10px;font-size:11.5px;font-weight:800;background:#fffbeb;border:1.5px solid #fde68a;color:#d97706;border-radius:10px;cursor:pointer">
                                    🔑 Reset
                                </button>

                                
                                <form action="<?php echo e(route('principal.staff.destroy', $staff)); ?>" method="POST" onsubmit="return confirm('Delete staff account for <?php echo e(addslashes($staff->name)); ?>?')" style="display:inline-block;margin:0">
                                    <?php echo csrf_field(); ?>
            ...
            <tr>
                <td colspan="4" style="text-align:center;padding:48px;color:#64748b">
                    ...
                    <a href="<?php echo e(route('principal.staff.create')); ?>" class="btn btn-primary" style="background:#4f46e5;color:#fff;border-radius:10px;font-weight:800">
...
<script>
    ...
    window.INSTITUTE_ACTIVE_FEATURES = <?php echo json_encode($activeFeatureKeys ?? [], 15, 512) ?>;
    ...
    <div class="modal-box" style="background:#ffffff!important;border:1px solid #e2e8f0!important;border-radius:18px!important;max-width:520px!important;width:100%!important;box-shadow:0 25px 60px -10px rgba(15,23,42,0.35)!important;overflow:hidden!important;position:relative!important;padding:0!important">
        ...
        <div style="padding:22px">
            ...
            <div style="background:#f0fdf4;border:1px solid #a7f3d0;border-radius:12px;padding:14px 16px;margin-bottom:16px;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap">
                ...
                <a href="<?php echo e(route('principal.bulk-import.sample', 'staff')); ?>" style="flex-shrink:0;display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:9px;background:#fff;border:1.5px solid #a7f3d0;font-size:12px;font-weight:700;color:#059669;text-decoration:none;white-space:nowrap">⬇️ Sample CSV</a>
            </div>

            <form method="POST" action="<?php echo e(route('principal.bulk-import.staff')); ?>" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make(auth()->check() && auth()->user()->isTeacher() ? 'layouts.app' : 'principal.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\UPLYFT\uplifyt\resources\views\principal\staff\index.blade.php ENDPATH**/ ?>