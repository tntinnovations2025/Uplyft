...
class TimetableChatService
{
    ...
    public function handle(string $message, int $instituteId, ?AcademicTerm $activeTerm): array
    {
        ...
        switch ($intent) {
            ...
            case 'room':
                return $this->doRoom($msg, $activeTerm, $sections, $subjects, $rooms);
            default:
                return $this->helpMessage();
        }
    }
    ...
    protected function detectIntent(string $msg): string
    {
        ...
        if ($this->hasAnyWord($msg, ['room', 'classroom', 'hall', 'venue', 'lab'])) {
            return 'room';
        }
    ...
    protected function doAdd(string $msg, AcademicTerm $term, Collection $sections, Collection $subjects, Collection $teachers, Collection $rooms): array
    {
        ...
        $validation = $this->conflictService->validateConflict(
            $term->id, $section->id, $teacher->id, $day, $start, $end, $room?->id, null, $subject->id
        );
    ...
    protected function canPlace(Timetable $slot, string $day, string $start, string $end, int $termId): bool
    {
        ...
        // Daily Subject Limit check: max 1 lecture of a subject per section per day
        $subjectClash = Timetable::where('academic_term_id', $termId)
            ->where('id', '!=', $slot->id)
    ...
    protected function doRoom(string $msg, AcademicTerm $term, Collection $sections, Collection $subjects, Collection $rooms): array
    {
        $section = $this->resolveSection($msg, $sections);
        if (!$section) {
            return $this->reply(false, false, 'Which class would you like to change/assign room for? For example: "Change room for Class 10A to Room 101" or "Set room for English in Class 10A to Room 202".');
        }

        $room = $this->resolveRoom($msg, $rooms);
        if (!$room) {
            return $this->reply(false, false, 'Which room would you like to assign? Please specify a registered room number, e.g. "Room 101" or "Room 202".');
        }

        $subject = $this->resolveSubject($msg, $subjects, $section);
        $day = $this->findDay($msg);

        // Scenario 1: Specific subject of class section
        if ($subject) {
            $query = Timetable::where('academic_term_id', $term->id)
                ->where('class_section_id', $section->id)
                ->where('subject_id', $subject->id);
            ...
            if ($slots->isEmpty()) {
                return $this->reply(false, false, "No scheduled lectures found for '{$subject->subject_name}' in {$this->sectionLabel($section)}" . ($day ? " on " . ucfirst($day) : '') . ".");
            }

            // Check room conflicts for the requested room
            foreach ($slots as $slot) {
                $clash = Timetable::where('academic_term_id', $term->id)
                    ->where('id', '!=', $slot->id)
                    ->where('room_id', $room->id)
                    ->where('day_of_week', $slot->day_of_week)
                    ->where(function ($q) use ($slot) {
                        $this->applyTimeOverlap($q, $slot->start_time, $slot->end_time);
                    })
                    ->first();

                if ($clash) {
                    return $this->reply(false, false, "❌ Room Conflict: Room {$room->room_number} is already occupied on " . ucfirst($slot->day_of_week) . " ({$slot->start_time} - {$slot->end_time}) by '{$clash->subject->subject_name}'.");
                }
            }

            // Update timetable slots & subject default room
            Timetable::whereIn('id', $slots->pluck('id'))->update(['room_id' => $room->id]);
            $subject->update(['room_id' => $room->id]);

            return $this->reply(true, true, "🚪 Updated room for <strong>{$subject->subject_name}</strong> in {$this->sectionLabel($section)} to <strong>Room {$room->room_number}</strong> across {$slots->count()} lecture slot(s).");
        }

        // Scenario 2: Whole class section room assignment
        $query = Timetable::where('academic_term_id', $term->id)
            ->where('class_section_id', $section->id);
        ...
        // Update timetable slots & class section default room
        Timetable::whereIn('id', $slots->pluck('id'))->update(['room_id' => $room->id]);
        $section->update(['room_id' => $room->id]);

        return $this->reply(true, true, "🏛️ Assigned <strong>Room {$room->room_number}</strong> to {$this->sectionLabel($section)} for all subjects across {$slots->count()} lecture slot(s).");
    }

    protected function resolveRoom(string $msg, Collection $rooms): ?Room
    {
        $n = strtolower($msg);

        // 1. Try regex pattern: "room 101", "room-101", "room101", "lab 1", "hall 2"
        if (preg_match('/(?:room|rm|lab|hall|venue|classroom)\s*[-_\s]?\s*([a-zA-Z0-9]+)\b/i', $n, $m)) {
            $target = strtolower($m[1]);
            foreach ($rooms as $r) {
                $rNum = strtolower($r->room_number);
                if ($rNum === $target || str_contains($rNum, $target) || $target === str_replace('room', '', $rNum)) {
                    return $r;
                }
            }
        }

        // 2. Direct match with any registered room number
        foreach ($rooms as $r) {
            $rNum = strtolower($r->room_number);
            if ($rNum !== '' && str_contains($n, $rNum)) {
                return $r;
            }
        }
    ...
    protected function helpMessage(): array
    {
        $help = "🤖 <strong>Schedule Assistant</strong> — I understand natural language. Here's what I can do:<br>"
            ...
            . "🚪 <strong>Change Room:</strong> \"Assign Room 101 to Class 10A\" or \"Set room for English in Class 10A to Room 202\"<br>"
